    ...
    super('quote', {
      ...
      args : [ {
        ...
        prompt : {
          ...
          // retry: 'That is not vaild user! Please provide a user ID or a
          // mention.'
        }
      } ]
    })
  ...
  exec(message, args) {
    ...
    function quote(msg, args, embed) {
      ...
      embed.setAuthor(`${msg.author.username}#${msg.author.discriminator}`,
                      `https://cdn.discordapp.com/avatars/${msg.author.id}/${
                          msg.author.avatar}.png`);
      embed.setDescription(`[View Message](https://discord.com/channels/${
          msg.guild.id}/${msg.channel.id}/${args.messageId})`);
      embed.addField('Content', msg.content);
      message.channel.send(embed)
    }

    let msglnk;
    if (args.messageId.includes('discord.com' ||
                                args.messageId.includes('/channels'))) {
      ...
      message.channel.messages.fetch(args.messageId)
          .then(msg => quote(msg, args, embed))
          .catch(console.error);
    }
  }
